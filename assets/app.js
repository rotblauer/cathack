...
function renderBuckets(buckets) {
  ...
  $.each(buckets, function (k,v) {
    console.log("BUCKET: " + JSON.stringify(v))
    h += '<div class="col-xs-12" id="bucket-' + v.meta.name + '" onClick="return getBucket(\'' + v.meta.name + '\')"><strong class="buck"><span class="octicon octicon-repo"></span>&nbsp;' + v.meta.name + '</strong></div>';
  });
