...
var writeBucketElem = $("#saveBucket");
...
function handleSnippetNameDidChange() {
...
function handleDestroySnippet() {
  ...
  if (reallyreally) {
    ...
    ajaxDeleteSnippet(ID, currentSnippet.bucketId);  
  }
...
function renderBuckets(buckets) {
  ...
  $.each(buckets, function (k,v) {
    h += '<div class="col-xs-12" id="bucket-' + v.meta.name + '" onClick="return getBucket(\'' + v.meta.name + '\')"><strong class="buck"><span class="octicon octicon-repo"></span>&nbsp;' + v.name + '</strong></div>';
  });
...
function ajaxDeleteSnippet(snippetId, bucketId) {
  return $.ajax({
    method: "DELETE",
    url: "/hack/s/" + snippetId + "?bucket=" + bucketId,
    error: function (e) {
...
snippetNameElem.on("input", handleSnippetNameDidChange);
// snippetMetaElem.on("input", sendUpdate(updateCurrentSnippetFromGUI()));

createSnippetElem.on("click", handleCreateSnippet());
destroySnippetElem.on('click', handleDestroySnippet()); 

writeBucketElem.on("click", saveTheBucket);
